...
export default function on_confirm(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Required_Item_Name(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Item_Name**: $.message.order.items[*].descriptor.name must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Fulfillment_Id**: $.message.order.provider.id must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Provider_Id**: $.message.order.fulfillments[*].id must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Payment_Transaction_Id(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Payment_Transaction_Id**: $.message.order.payments[*].params.transaction_id must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Payment_Currency(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Payment_Currency**: $.message.order.payments[*].params.currency must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Payment_Amount(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Payment_Amount**: $.message.order.payments[*].params.amount must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Break_Up_Item_Price_Currency(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Break_Up_Item_Price_Currency**: $.message.order.quote.breakup[*].item.price.currency must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Created_At(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Created_At**: $.message.order.created_at must be present in the payload`,
                        },
                    ];
                }
        ...
        function Required_Updated_At(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
  ...
  - **condition Fullfillment_Type_ENUM.2**: $.message.order.fulfillments[*].type must be present in the payload`,
	...
	> - **condition B**: $.message.order.fulfillments[*].state.descriptor.code must **not** be present in the payload`,
        ...
        function ORDER_PAYMENT_STATUS_ENUM(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition ORDER_PAYMENT_STATUS_ENUM**: all of the following sub conditions must be met:
  ...
  - **condition Order_Status_ENUM.2**: $.message.order.status must be present in the payload`,
        ...
        function ROUTE_ENUM(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition ROUTE_ENUM**: every element of $.message.order.fulfillments[*].state.descriptor.code must be in ["ROUTE_INFO", "TICKET_INFO", "TRIP_DETAILS", "INFO"]
	...
	> - **condition B**: $.message.order.payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        const testFunctions: testFunctionArray = [
            ...
            Required_Payment_Id,
            Required_Payment_Transaction_Id,
            ...
            Required_Break_Up_Item_Price_Currency,
