...
export default function select(input: validationInput): validationOutput {
  ...
  - **condition REQUIRED_CONTEXT_DOMAIN.2**: every element of $.context.domain must be in ["ONDC:RET10", "ONDC:RET11", "ONDC:RET12", "ONDC:RET13", "ONDC:RET14", "ONDC:RET15", "ONDC:RET16", "ONDC:RET17", "ONDC:RET18", "ONDC:RET19"]`,
        ...
        function REQUIRED_MESSAGE_GPS(
            ...
            for (const testObj of scope) {
                ...
                const attr = payloadUtils.getJsonPath(
                    testObj,
                    "$.message.order.fulfillments[*].end.gps",
                );
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition REQUIRED_MESSAGE_GPS**: $.message.order.fulfillments[*].end.gps must be present in the payload`,
                        },
                    ];
                }
        ...
        function REQUIRED_MESSAGE_AREA_CODE(
            ...
            for (const testObj of scope) {
                ...
                const attr = payloadUtils.getJsonPath(
                    testObj,
                    "$.message.order.fulfillments[*].end.address.area_code",
                );
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition REQUIRED_MESSAGE_AREA_CODE**: $.message.order.fulfillments[*].end.address.area_code must be present in the payload`,
                        },
                    ];
                }
        ...
        function REQUIRED_MESSAGE_PAYMENT(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition REQUIRED_MESSAGE_PAYMENT**: all of the following sub conditions must be met:
  ...
  - **condition REQUIRED_MESSAGE_PAYMENT.2**: every element of $.message.order.payment must be in ["ON-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]`,
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition REQUIRED_MESSAGE_ID_22**: $.message.order.offers[*].id must be present in the payload`,
                        },
                    ];
                }
	...
	> - **condition B**: $.context.domain must **not** be present in the payload`,
        ...
        function VALID_ENUM_MESSAGE_CODE(
            ...
            for (const testObj of scope) {
                ...
                const enumPath = payloadUtils.getJsonPath(
                    testObj,
                    "$.message.order.offers[*].tags.code",
                );
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_CODE**: every element of $.message.order.offers[*].tags.code must be in ["Disc_Pct", "Disc_Amt", "BuyXGetY", "Freebie"]
	...
	> - **condition B**: $.message.order.offers[*].tags.code must **not** be present in the payload`,
        ...
        function VALID_ENUM_MESSAGE_PAYMENT(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_PAYMENT**: every element of $.message.order.payment must be in ["ON-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]
