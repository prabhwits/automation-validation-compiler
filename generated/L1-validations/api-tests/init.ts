...
export default function init(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function Required_Collected_By(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Required_Collected_By**: $.message.order.payments[*].collected_by must be present in the payload`,
                        },
                    ];
                }
  ...
  - **condition Required_Domain_and_ENUM.2**: $.context.domain must be present in the payload`,
	...
	> - **condition B**: $.message.order.payments[*].tags[*].descriptor.code must **not** be present in the payload`,
        ...
        function Tags_ENUM_for_BUYER_FINDER_FEES(
            input: validationInput,
        ): validationOutput {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Tags_ENUM_for_BUYER_FINDER_FEES**: every element of $.message.order.payments[*].tags[?(@.descriptor.code=='BUYER_FINDER_FEES')].list[*].descriptor.code must be in ["BUYER_FINDER_FEES_PERCENTAGE", "BUYER_FINDER_FEES_AMOUNT"]`,
                        },
                    ];
                }
        ...
        function Tags_ENUM_for_SETTLEMENT_TERMS(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(
                input.payload,
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition Tags_ENUM_for_SETTLEMENT_TERMS**: every element of $.message.order.payments[*].tags[?(@.descriptor.code=='SETTLEMENT_TERMS')].list[*].descriptor.code must be in ["SETTLEMENT_WINDOW", "SETTLEMENT_BASIS", "SETTLEMENT_TYPE", "MANDATORY_ARBITRATION", "COURT_JURISDICTION", "DELAY_INTEREST", "STATIC_TERMS", "SETTLEMENT_AMOUNT"]`,
                        },
                    ];
                }
        ...
        const testFunctions: testFunctionArray = [
            ...
            Tag_Group_ENUM,
            Tags_ENUM_for_BUYER_FINDER_FEES,
            Tags_ENUM_for_SETTLEMENT_TERMS,
        ];
