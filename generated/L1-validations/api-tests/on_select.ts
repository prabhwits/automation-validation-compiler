...
export default function on_select(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function validate_category_ids_27(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_category_ids_27**: $.message.order.items[*].category_ids[*] must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_type_30**: $.message.order.fulfillments[*].type must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_currency_31**: $.message.order.quote.breakup[*].item.price.currency must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_value_32**: $.message.order.quote.breakup[*].item.price.value must be present in the payload`,
                        },
                    ];
                }
        ...
        function validate_count_33(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_count_33**: $.message.order.quote.breakup[*].item.quantity.selected.count must be present in the payload`,
                        },
                    ];
                }
        ...
        function validate_url_34(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_url_34**: $.message.order.cancellation_terms[*].external_ref.url must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_mimetype_35**: $.message.order.cancellation_terms[*].external_ref.mimetype must be present in the payload`,
                        },
                    ];
                }
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_enum_1**: every element of $.context.action must be in ["on_select"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition validate_enum_13**: every element of $.message.order.quote.breakup[*].title must be in ["BASE_FARE", "REFUND", "CANCELLATION_CHARGES", "OFFER", "TOLL"]
