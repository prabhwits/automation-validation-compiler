...
const main = async () => {
	...
	const buildPath = path.resolve(__dirname, "../samples2/build.yaml");
	const valConfigPath = path.resolve(
		__dirname,
		"../samples2/validation-config.json"
	);
	...
	// 	output += `\n## ${input}\n\n${out}\n`;
	// }
	// writeFileSync(path.resolve(__dirname, "../samples2/output.md"), output);
};
